...
test('dry-run 生成三种成品且绝不调用发信', async () => {
  ...
  const config = { dryRun: true, sampleFallback: false, requireChineseTranslation: false, timezone: 'Asia/Shanghai', maxGlobal: 10, maxChina: 5, maxUsa: 5, outputDir, historyFile: path.join(root, 'history.json'), keywords, topics, requestedTopic: 'technology-energy', reliability: { Test: 0.9, default: 0.7 }, ai: { provider: 'auto', geminiKey: '', groqKey: '' }, email: { user: '', password: '', to: [] } };
...
test('测试邮件绕过当天正式锁且不会覆盖锁或历史', async () => {
  ...
  const config = { dryRun: false, sampleFallback: false, requireChineseTranslation: false, timezone: 'Asia/Shanghai', maxGlobal: 10, maxChina: 5, maxUsa: 5, outputDir, historyFile, keywords, topics, requestedTopic: 'technology-energy', reliability: { Test: 0.9, default: 0.7 }, ai: { provider: 'auto', geminiKey: '', groqKey: '' }, email: { user: 'sender@gmail.com', password: 'app-password', to: ['reader@example.com'], testMode: true } };
...
test('英文新闻翻译失败时禁止发送残缺日报', async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'digest-translation-guard-')); const outputDir = path.join(root, 'output'); fs.mkdirSync(outputDir);
  const now = new Date('2026-01-02T00:00:00Z');
  const config = { dryRun: false, sampleFallback: false, requireChineseTranslation: true, timezone: 'Asia/Shanghai', maxGlobal: 10, maxChina: 5, maxUsa: 5, outputDir, historyFile: path.join(root, 'history.json'), keywords, topics, requestedTopic: 'technology-energy', reliability: { Test: 0.9, default: 0.7 }, ai: { provider: 'auto', geminiKey: '', groqKey: '' }, email: { user: 'sender@gmail.com', password: 'app-password', to: ['reader@example.com'], testMode: true } };
  const article = { id: '1', title: 'International leaders discuss security', source: 'Test', url: 'https://example.com/a', publishedAt: '2026-01-01T23:00:00Z', summary: 'Officials announced new international talks.', crossSources: [] };
  let mailed = false;
  await assert.rejects(() => run({ config, now, fetchers: { rss: async () => [article], gdelt: async () => [] }, mailer: async () => { mailed = true; return { messageId: 'should-not-send' }; } }), /中文翻译未完成/);
  assert.equal(mailed, false);
  assert.equal(fs.existsSync(path.join(outputDir, 'latest.html')), false);
});
