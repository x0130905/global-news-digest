...
async function gemini(article, config) {
  const endpoint = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${encodeURIComponent(config.ai.geminiKey)}`;
  ...
  const r = await fetchWithRetry(endpoint, { timeoutMs: config.timeoutMs, method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body), retries: 2 });
