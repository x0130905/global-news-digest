...
test('PWA 页面包含搜索、响应式视口和安全外链处理', () => {
  ...
  assert.match(html, /id="search"/); assert.match(html, /id="date-picker"/); assert.match(html, /side-panel/); assert.match(html, /viewport/); assert.match(html, /manifest\.webmanifest/); assert.match(app, /archive\/index\.json/); assert.match(app, /sources-panel/); assert.match(app, /card-source/); assert.match(app, /noopener noreferrer/); assert.match(app, /http:.*https:/); assert.match(app, /bilingual-grid/); assert.match(app, /EN · ORIGINAL/); assert.match(app, /中文翻译/);
  assert.match(html, /<aside class="side-panel"[\s\S]*id="search"[\s\S]*id="filter-list"[\s\S]*<\/aside>/); assert.doesNotMatch(html, /id="topic-banner"|id="overview"|sticky-topic|DAILY FOCUS/);
  assert.match(html, /data-filter="label:red"/); assert.match(html, /data-filter="label:blue"/); assert.match(html, /data-filter="label:green"/); assert.match(app, /global-pulse-article-labels-v1/); assert.match(app, /data-label-action/); assert.match(css, /--label-red/); assert.match(css, /--label-blue/); assert.match(css, /--label-green/); assert.match(css, /\.side-panel\{position:sticky/); assert.match(css, /\.side-panel \.filter-scroll\{display:flex;flex-direction:column/);
});
